...
const static char *TAG = "SMTP";



#define TASK_STACK_SIZE     (8 * 1024)
//#define BUF_SIZE            512
#define BUF_SIZE            LOG_BUFFER_SIZE+100
...
void sendEmail(){
    ...
    ESP_LOGI(TAG, "Loading the CA root certificate. %d",server_root_cert_pem_end - server_root_cert_pem_start);
    ...
    ESP_LOGI(TAG, "Setting hostname to %s for TLS session...", MAIL_SERVER);
    ...
    ESP_LOGI(TAG, "Write USER NAME %s",SENDER_MAIL);
    ...
    len = snprintf((char *) buf, BUF_SIZE,
                   "From: %s\r\nSubject: room monitoring restarted\r\n"
                   "To: %s\r\n"
                   "MIME-Version: 1.0 (mime-construct 1.9)\n",
                   SENDER_MAIL, RECIPIENT_MAIL);
